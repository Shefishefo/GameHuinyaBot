...
# bot init
bot = Bot(token='5384876083:AAGWry0mHgsOb2_s-zBpLTNtTCoO0qvlon0')
dp = Dispatcher(bot)
...
###########################################АДМИН КОМАНДЫ###########################################
    if message.text.startswith("Поделить"):
       ...
       if user_status[0] == 'Admin':
          ...
          await message.reply(f"<a href='tg://user?id={user_id}'>{user_name}</a> ,Вы не являетесь администратором бота 👮. Для получение прав администратора обратитесь к разработчику @Sofa_sex ⚠️", parse_mode='html')
...
Разработчик: @Sofa_sex 💻
Наша беседа: <tg-spoiler>@@gamer_zoneee</tg-spoiler> 💬 
...
ℹ️ | Чтобы купить Donate-coins, обратитесь к владельцу бота @Sofa_sex
...
async def craft_resurs3(callback: types.CallbackQuery):
   ...
   if user_status in ['Admin', 'Helper_Admin', 'Owner']:
      ...
      await callback.message.answer( f"<a href='tg://user?id={user_id}'>{user_name}</a>, вы не являетесь администратором бота 👮. Для получение прав администратора обратитесь к разработчику @Sofa_sex ⚠️", parse_mode='html')
...
🛒 Для покупки администраторских прав , обратитесь к Владельцу бота - @Sofa_sex
    """,  parse_mode='html' )
